  ...
  try {
    ...
    if (!password.match(passwordRegex)) {
      return res.status(200).json({
        payload:
          "Password should contain at least 6 characters. An uppercase letter, lowercase letter, number, and a special character.",
      });
    } else {
      ...
      if (userResponse) {
        return res
          .status(200)
          .json({ payload: "User with this email already exists." });
      } else {
    ...
    return res
      .status(500)
      .json({ payload: "Something went wrong, please try again later" });
  }
};
  ...
  try {
    ...
    if (signInResponse.errMsg) {
      ...
      if (signInResponse.role === "user") {
        ...
        res
          .status(200)
          .json({ payload: signInResponse.successMessage, token: token });
      }
    }
  } catch (error) {
    console.error(error);
...
const googleSignin = async (
  ...
  try {
    ...
    if (userExists) {
      ...
      return res
        .status(200)
        .json({ payload: "Sign in successful", token: token });
    } else {
      const newUser = new userModel({
        email: email.toLowerCase(),
        fullName: fullName,
  ...
  try {
    ...
    const doctorSchema = new doctorModel({
      profile: {
        ...
        doctorImage: renamedResult.secure_url,
      },
      doctorDepartment,
  ...
  try {
    ...
    if (uploadedFile) {
      ...
      if (updateDoctor.modifiedCount > 0) {
        return res
          .status(200)
          .json({ payload: "Doctor details updated successfully" });
      } else {
      ...
      if (updateDoctor.modifiedCount > 0) {
        return res
          .status(200)
          .json({ payload: "Patient details updated successfully" });
      } else {
        return res.status(404).json({ payload: "No changes made" });
      }
    }
...
const deleteDoctor = async (
  ...
  try {
    ...
    const deleteDoctorCall = await doctorModel.deleteOne({
      doctorID: doctorID,
    });
  ...
  try {
    ...
    const savePatient = new patientModel({
      profile: {
        ...
        patientImage: renamedResult.secure_url,
      },
      patientID: patientID,
...
const createPatientNote = async (
  ...
  try {
    ...
    if (createPatientNote.modifiedCount === 0) {
      return res
        .status(404)
        .json({ payload: "Patient not found or note not added" });
    }
...
const deletePatient = async (
  ...
  try {
    ...
    const findPatientRoom = await admissionsModel.findOne({
      occupantID: patientID,
    });

    if (findPatientRoom) {
      return res.status(404).json({
        payload: "Patient still checked in!",
      });
    }

    const patientImageID = `patientImages/${patientID}`;

    const deletePatientCall = await patientModel.deleteOne({
      patientID: patientID,
    });
...
const checkInPatient = async (
  ...
  try {
    ...
    if (!findPatient) {
      return res
        .status(404)
        .json({ payload: "Patient not found, contact support." });
    }
...
const checkOutPatient = async (
  ...
  try {
    ...
    return res
      .status(200)
      .json({ payload: "Patient checked out successfully" });
  } catch (error) {
    return res
      .status(500)
      .json({ payload: "Something went wrong, please try again later" });
