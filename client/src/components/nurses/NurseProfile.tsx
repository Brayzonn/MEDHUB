...
interface NurseProfileListProps{
    header: string ,
    data: string
}
interface NurseProfileProps {
    nurseData: NurseProfileListProps[],
    updateNurseEditState: React.Dispatch<React.SetStateAction<boolean>>,
    nurseEditState: boolean,
    isNurseProfileVisible: boolean,
    updateNurseProfileVisibility: React.Dispatch<React.SetStateAction<boolean>>,
}

const NurseProfile:React.FC<NurseProfileProps> = ({isNurseProfileVisible, updateNurseProfileVisibility, nurseData, updateNurseEditState, nurseEditState}) => {
  const [isConfirmationDialogOpen, setIsConfirmationDialogOpen] = useState(false);

  return (
    <>
    {(isNurseProfileVisible && !nurseEditState) && <div className="z-50 fixed top-0 left-0 w-full min-h-full flex justify-center items-center text-[#161616]">
          ...
          <div className="shadow-2xl relative px-6 py-8 w-[95%] h-[100%] bg-white border border-[#f7f7f7] rounded-[15px]">
                  ...
                  <ConfirmationDialog
                        ...
                        title="Do you want to delete Nurse profile?"
                  ...
                  <div className="w-full min-h-full flex flex-col space-y-[1rem]">
                        ...
                        <div className='w-full flex justify-end items-end space-x-4'>
                                <button onClick={()=> updateNurseProfileVisibility(false)} className="transition-properties w-[130px] h-[40px] bg-black text-white border border-black text-[14px] rounded-md flex items-center justify-center space-x-2 hover:border-[#121212] hover:bg-[#121212]">
                                      <FaChevronDown className ='text-white text-[14px]'/>
                                      <p>Close</p>
                                </button>

                                <button onClick={()=>{updateNurseEditState(true); window.scrollTo(0, 400);}} className="transition-properties w-[130px] h-[40px] text-white border bg-yellow-500 text-[14px] border-yellow-500 rounded-md flex items-center justify-center space-x-2 hover:border-yellow-400 hover:bg-yellow-400">
    ...
    {(isNurseProfileVisible && nurseEditState) && <EditNurse updateNurseProfileForm ={updateNurseProfileVisibility} updateEditNurseForm={updateNurseEditState}/>}
    </>
  )
}
